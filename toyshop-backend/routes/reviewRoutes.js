...
const express = require("express");
const Review = require("../models/Review");
const upload = require("../utils/upload"); // ✅ Import Cloudinary multer setup

const router = express.Router();

// ✅ Create a new review (upload to Cloudinary)
router.post("/", upload.array("images", 5), async (req, res) => {
  const { productId, userId, rating, comment } = req.body;
...
// ✅ Update a review (with new Cloudinary images if provided)
router.put("/:reviewId", upload.array("images", 5), async (req, res) => {
  const { reviewId } = req.params;
  const { rating, comment } = req.body;
  const images = req.files ? req.files.map((file) => file.path) : undefined; // ✅ Cloudinary URLs
